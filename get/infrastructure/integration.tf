...
resource "aws_api_gateway_integration" "integration" {
  ...
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:${data.aws_region.current.name}:lambda:path/2015-03-31/functions/${aws_lambda_function.get.arn}:$${stageVariables.lambdaAlias}/invocations"
}
